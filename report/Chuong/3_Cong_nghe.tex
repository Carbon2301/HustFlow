...
Trong HustFlow, bài toán phát hiện chu trình được giải quyết bằng thuật toán duyệt đồ thị theo chiều sâu (Depth-First Search - DFS) kết hợp cơ chế đánh dấu trạng thái đỉnh. Mỗi đỉnh có ba trạng thái: chưa thăm, đang thăm và đã thăm xong. Nếu trong quá trình duyệt từ một đỉnh, thuật toán gặp lại một đỉnh đang nằm trên nhánh duyệt hiện tại, đồ thị tồn tại cạnh ngược và quan hệ phụ thuộc mới bị từ chối. Thuật toán có độ phức tạp thời gian $O(V + E)$ và độ phức tạp không gian $O(V)$, phù hợp với quy mô một bảng công việc trong hệ thống.

Một hướng tiếp cận thay thế là thuật toán Kahn dựa trên sắp xếp topo. Thuật toán này liên tục loại bỏ các đỉnh có bán bậc vào bằng 0; nếu sau quá trình loại bỏ vẫn còn đỉnh chưa được xử lý thì đồ thị có chu trình. Cả DFS và Kahn đều có độ phức tạp tuyến tính theo số đỉnh và cạnh. Tuy nhiên, DFS được lựa chọn trong đồ án vì có thể dừng ngay khi phát hiện cạnh ngược đầu tiên, dễ triển khai trong luồng kiểm tra khi thêm một quan hệ phụ thuộc mới và không yêu cầu tính lại toàn bộ thứ tự topo của bảng. Cách lựa chọn này đáp ứng trực tiếp yêu cầu về độ tin cậy dữ liệu ở phần \ref{subsection:2.4.3} và yêu cầu phản hồi nhanh ở phần \ref{subsection:2.4.1}.
